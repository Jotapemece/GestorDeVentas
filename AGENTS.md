...
- **Marquee de texto truncado (2026-08-20)**: **128/128** vitest + `node --check src/*.js` OK + minify OK (17 JS + 2 CSS + HTML). Plan aprobado por el usuario ("Apruebo, en cuanto a pc solo una vez por hover y que regrese al estado en que estaba cuando se posa el mouse sobre otro"). Solo frontend (sin tocar Rust).
  ...
  - **Helper `initMarquee()`** (utils.js): delegación en `document` (capture). `pointerover`+`pointerout` con `pointerType==='mouse'` (guarda `mqHover` para no re-disparar dentro del mismo hover; al cambiar de elemento `mqStop()` restaura el anterior). `pointerdown` no-mouse para móvil. `animationend` sobre `.mq-inner` limpia (móvil queda quieto). `mqStart` mide desborde, envuelve el contenido en `<span class="mq-inner">`, setea `--mq-dist` negativo y añade `.mq-anim`; `mqStop` desempaqueta y restaura el DOM original (incluidos badges).
  - **CSS** (style.css): `.mq-inner` (inline-block, nowrap, will-change), `.mq-anim .mq-inner` (animación `mq-roll` 2.4s ease-in-out 1), `@keyframes mq-roll` (0→--mq-dist→0→--mq-dist→0). `.product-card-name` móvil gana `overflow:hidden; text-overflow:ellipsis; white-space:nowrap; flex:1 1 auto; min-width:0` (antes el nombre envolvía a 2 líneas, ahora trunca para poder rodar). `prefers-reduced-motion` global ya anula la animación.
  - **Tests**: +6 vitest en dom.test.js (no rueda si cabe, envuelve con distancia correcta, mqStop restaura DOM, no re-dispara mismo elemento, al pasar a otro restaura el anterior, tolerancia 2px). `initMarquee()` se llama en el DOMContentLoaded de app.js.
- **Adaptación móvil A–H (2026-08-20)**: **122/122** vitest + `node --check src/*.js` OK + minify OK (17 JS + 2 CSS + HTML). Plan aprobado por el usuario ("Apruebo, anotalo actualiza el TODO y empieza") con 8 ítems, todos frontend (sin tocar Rust). Detalle:
  - **A — `data-label` en filas de alertas y solicitudes**: `createAlertaRow` (alerts-view.js) con data-label Fecha/Usuario/Tipo/Cliente/Monto/Método/Nota y `createSolicitudRow` (solicitudes-view.js) con Fecha/# Venta/Solicitante/Motivo/Estado/Acciones → el CSS móvil `.table-cards` (solo ≤600px, style.css @media 2939) convierte las tablas de los modales en cards legibles en móvil.
  - **B — `showSaleDetail` móvil**: la tabla del detalle de venta (reports-view.js:901) pasa a `class="table table-cards"` con `data-label` (Producto/Cantidad/Precio Unit./Subtotal/Estado/Acción).
  - **C — `drawProfitLineChart` con fallback**: si el filtro (default hoy→hoy) da <2 puntos, re-consulta últimos 30 días con flag `usedFallback` y etiqueta "últimos N días (filtro actual sin datos)"; altura móvil `CHART.BAR_HEIGHT_MOBILE` (240) cuando `rect.width < BREAKPOINT.MOBILE` (500). Arregla el "Datos insuficientes" constante al entrar a Reportes.
  - **D — CSS móvil del select "Productos más vendidos"**: en `@media` móvil `#top-products-limit` gana `flex-basis:100%; width:100% !important` y `.config-card-desc` `flex:1 1 100%` → el select no se corta en la card colapsable.
  - **E — Preview de cambios al login (PC + Android)**: `openDownloadPreviewModal(opts)` (app.js) acepta `{ showIfEmpty }` (si no hay cambios no muestra el modal) y se expone como `window.openLoginDownloadPreview`. `runLoginSync` (views.js) ahora abre el preview al iniciar sesión (antes `invoke('download_all')` y un `if (IS_ANDROID) return` que dejaba a Android sin bajar nada al login). Fallback a `download_all` si la función global no existe.
  - **F — Splash por tema**: splash (style.css) usa `--sidebar-text` y `rgba(var(--sidebar-text-rgb), .45)` para icono/título/cursor/subtítulo → en Rubí (fondo `#B42A2A`) ya no se ve negro sobre rojo. Añadido `--sidebar-text-rgb: 74,68,88` al `:root` default.
  - **G — Transición de tema**: overlay `#theme-sweep` (fixed, z-index 8000, pointer-events none) con gradiente vertical `--primary→--accent`, animado `translateY(-100%)→0` ~0.55s ease-out con fade in/out (la línea barre de arriba a abajo mostrando el nuevo color); clase temporal `body.theme-anim` aplica `transition` de background/color/border-color a selectores base durante ~450ms. `runThemeTransition()` (config-view.js) se dispara SOLO en `handleThemeClick` (no en arranque ni `loadThemeConfig`). Respeta `prefers-reduced-motion` (media query global ya pone animaciones a 0s).
  - **H — Fix modal/teclado Android**: causa raíz: con `adjustResize` (manifest) el WebView ya se encoge solo, pero el handler `visualViewport` (app.js) seguía añadiendo `paddingBottom` interno al `.modal-content` → doble empuje que separaba el footer sticky del resto (franja cortada al medio). Ahora calcula el **overlap real** (`innerHeight - visualViewport.height`) y solo empuja si `overlap > KEYBOARD.PAD_OFFSET`; en modales móviles aplica `maxHeight: calc(92dvh - …)` al `.modal-content` (encoge el scrollport, el footer se queda pegado abajo) en vez de padding interno. Limpieza de ambos al cerrar el teclado.
- **Fix "aparecen más cosas al reabrir la app" — plan A–D (2026-08-20)**: **188/188** Rust + `cargo check` OK + **122/122** vitest + `node --check` OK + minify OK (17 JS). Fix del bug de la laptop: al reabrir la app aparecían items que no se habían mostrado en la sincronización anterior (productos/clientes/ventas que reaparecen en el preview cada vez). Diagnóstico verificado contra el código y Supabase (esquemas, paginación, dist, llamadores). Plan aprobado por el usuario ("Apruebo, actualiza el TODO y empieza").
  - **A — No duplicar el descuento de stock al aplicar ventas**: `apply_remote_sales` (sync/sales.rs) ya NO ajusta stock al descargar ventas remotas (antes `sub_stock`/`add_stock` por cada detalle). Causa: `download_products_inner`/`apply_download` ya aplican el `stock` remoto (que refleja esas ventas), así que el ajuste por venta era un **doble descuento**, y el bump de `updated_at` de `sub_stock`/`add_stock` (db.rs:255-270) dejaba el producto local "más nuevo" que el remoto → el LWW lo saltaba para siempre y el diff de stock reaparecía en el preview en cada apertura. Ahora el stock viaja SOLO por `productos.stock` (upload/download de productos); las anulaciones remotas propagan la restauración de stock vía el producto del dispositivo que anuló (`void_sale` → `add_stock` → bump → upload). Se conserva la reconciliación de estados de anulado (flags) con `anulado_delta` (ahora solo devuelve el estado objetivo `bool`; se eliminaron `items_consumed`/`items_restored` de los contadores y mensajes).
  - **B — No bumpear `updated_at` en `sub_stock`/`add_stock` durante sync**: cubierto por A — ya no quedan llamadas a `sub_stock`/`add_stock` desde el módulo sync (solo `sales.rs`: create_sale/void_sale/void_sale_items, operaciones locales que SÍ deben bumpear para propagar stock).
  - **C — `apply_download` respeta `first_sync`**: preview.rs (productos y clientes) ahora aplica LWW off en el primer sync (`CFG_FIRST_SYNC_DONE` vacío), igual que ya hacían `apply_remote_sales`/`download_products_inner`/`download_clientes_inner`. Antes el modal de descarga selectiva en un dispositivo nuevo omitía items con `remote_ts <= local_ts` (que reaparecían).
  - **D — Paginación estable en `supabase_get_paginated`**: nuevo parámetro `tie_breaker` (col PK de la tabla remota: `codigo` en productos, `id` en ventas/detalles/clientes/alertas/solicitudes, `sync_id` en usuarios) → `order=updated_at.asc,{tie_breaker}.asc`. Sin él, los empates de `updated_at` (994 de 1185 detalles comparten el mismo ts en el borde de página) hacían inestable el cursor `offset` (saltos/duplicados entre páginas). Verificado en vivo contra Supabase: 0 solapamientos entre páginas 1 y 2.
  - **Tests**: `anulado_delta` actualizado a firma de 2 args (`remote_anulado, venta_anulada → bool`) con tests ajustados; comentario en solicitudes.rs:128 corregido (la restauración de stock ahora viaja por el producto del dispositivo que anuló).
...
- **Tetris (solo PC)**: `src/tetris.js` (nuevo, reemplaza `src/snake.js`): juego "Tetris" con gráficos monocromos renderizados en `<canvas>` (estilo Game Boy: cada pieza tiene un patrón de bloque propio para distinguirse sin color). Lógica pura (`tetrisCreate/tetrisSpawn/tetrisStep/tetrisRotate/tetrisHardDrop/tetrisClearLines/...`) separada de la integración DOM. 7 piezas con 4 estados de rotación (matriz 4×4), 7-bag, ghost/sombra, preview "Siguiente", HUD Puntos/Líneas/Nivel, velocidad que sube de nivel cada 10 líneas. Tamaños S/M/L. Acceso desde la Guía rápida (tab "Juego" + `#guide-juego` con botón `#tetris-btn`, oculto en Android vía `IS_ANDROID`). Modal `#tetris-modal`, teclado capture+stopImmediatePropagation (Flechas mover/bajar, ↑/X rotar CW, Z rotar CCW, Espacio hard drop, P pausa, R reiniciar), guard en `tetrisDoTick` si el modal se cierra. SEL de tetris en constants.js, estilos `.snake-*`/`.tetris-*` en style.css, `initTetris()` llamada en views.js. Tests: `src/__tests__/tetris.test.js` (20 de lógica pura).
- **Fase 1 completa**:
...
### Active
- **Optimización + nuevo logo (plan APROBADO 2026-08-27, COMPLETADO solo local — sin commit/push)**: limpieza de código muerto, índices BD, `get_profit_series` N+1, paginación de clientes/movimientos, `productCache`, `preview_download` delta, `get_config_bulk`, fugas frontend, `upsert_config`; más nuevo icono de app (PC + Android) desde `logonuevo.jpeg` (fondo transparente). Decisiones del usuario: optimización **COMPLETA** + paginación con botón **"Cargar más"**; logo con fondo **TRANSPARENTE**; entrega **SOLO LOCAL** (sin commit/push). Plan por fases:
  - **Fase 0 — Código muerto**: borrar CSS `.cart-edit-price-btn`/`.cart-price-edit`; borrar `#product-costo-error`/`#product-stock-minimo-error` de index.html.
  - **Fase 1 — Índices BD (migración 043)**: `detalles_ventas(venta_id)`, `ventas(fecha_hora)`, `movimientos_caja(created_at)`, `ventas(dispositivo_origen)`, `clientes(activo)`, `productos(activo)`, `ventas(anulada)`. (Los `updated_at` ya en migración 035.) Verificar con `EXPLAIN QUERY PLAN`.
  - **Fase 2 — `get_profit_series`**: reescribir con CTE `costo_por_dia` agrupado (sin duplicar `SUM(v.total_usd)`). + test.
  - **Fase 3 — Paginación `list_clientes`/`list_movimientos`**: `list_clientes` SIEMPRE con `page/page_size`; nuevo `get_clientes_resumen` → `{total, con_deuda, deuda_total, personas_con_deuda}` para stats; `list_movimientos(page, page_size)` `ORDER BY id DESC LIMIT ? OFFSET ?`. Frontend "Cargar más" en `clients-view.js` y `cashier-view.js`; quitar `list_clientes` completo de `shortcuts.js` y chat Enar (`views.js`).
  - **Fase 4 — `productCache`**: no recargar tras venta (actualizar `stock` in-place + `refreshEfectivoSaldo`); fallback `list_productos(search)` si no está en cache truncada (tope 5000).
  - **Fase 5 — `preview_download` delta**: filtrar `updated_at > watermark` (todas solo en `first_sync`).
  - **Fase 6 — `get_config_bulk`**: comando `get_config_values(keys) -> HashMap`; refactor login/`showView`.
  - **Fase 7 — Fugas frontend**: listeners de toast removidos en `hideToast`; `clearInterval(syncRelInterval)` en `handleLogout`.
  - **Fase 8 — `upsert_config`**: devolver `Result` y propagar en callers de sync.
  - **Logo**: `convert logonuevo.jpeg -resize 1024x1024 -background none -gravity center -extent 1024x1024 logonuevo_1024.png` → `npm run tauri -- icon logonuevo_1024.png` (regenera `icons/` PC + `mipmap-*` Android). No tocar UI interna ni `tauri.conf.json`. Limpiar PNG intermedio.
  - **Verificación**: `cargo test --lib`, `cargo check`, `EXPLAIN QUERY PLAN`, `node --check src/*.js`, `node scripts/minify.mjs`, `npx vitest run`. Sin commit (solo local).
  - **Resultado (2026-08-27)**: **195/195** `cargo test --lib` + `cargo check` OK, **135/135** vitest (`npx vitest run`), `node --check src/*.js` OK, `node scripts/minify.mjs` OK (17 JS + 2 CSS + HTML). Índices en migración 043 aplicados; `get_profit_series` con CTE `costo_por_dia` (sin N+1); `list_clientes`(search+page)/`list_movimientos`(page) paginados + `get_clientes_resumen`; `productCache` actualizado in-place tras venta con fallback `list_productos`; `preview_download` filtra por watermark salvo `first_sync`; `get_config_values` (HashMap) con refactor de clusters en login/`showView`; fuga de listeners de toast corregida en `hideToast` + `clearInterval(syncRelInterval)` en `handleLogout`; `upsert_config` devuelve `Result` y propaga (`?`) en 24 call sites de sync (cargo check limpio, 0 warnings). Logo: `logonuevo_1024.png` (RGBA transparente) → `npm run tauri -- icon` regeneró `icons/` (PC), iOS y `mipmap-*` (Android); PNG intermedio eliminado. Todo **solo local (sin commit/push)**.
