...
pub fn list_products(
    ...
    let base_select = "SELECT p.codigo, p.nombre, p.precio_usd, p.stock, COALESCE(p.stock_minimo,0), COALESCE(p.created_at,''), p.categoria_id, c.nombre, c.color FROM productos p LEFT JOIN categorias c ON c.id = p.categoria_id";
    let has_query = search.as_ref().is_some_and(|s| !s.is_empty());

    let (sql, _param_count): (String, usize) = match (has_query, categoria_id) {
        (true, Some(_cat)) => (
            format!("{} WHERE (p.codigo LIKE ?1 OR p.nombre LIKE ?1) AND p.categoria_id = ?2 ORDER BY p.nombre ASC", base_select),
            2,
        ),
        (true, None) => (
            format!("{} WHERE p.codigo LIKE ?1 OR p.nombre LIKE ?1 ORDER BY p.nombre ASC", base_select),
            1,
        ),
        (false, Some(_cat)) => (
            format!("{} WHERE p.categoria_id = ?1 ORDER BY p.nombre ASC", base_select),
            1,
        ),
        (false, None) => (
...
pub fn create_product(
    ...
    match db.execute(
        "INSERT INTO productos (codigo, nombre, precio_usd, stock, categoria_id, created_at) VALUES (?1, ?2, ?3, ?4, ?5, datetime('now','localtime'))",
...
pub fn export_products_xlsx(state: State<AppState>, tasa: f64) -> Result<String, String> {
    ...
    let mut stmt = db
        .prepare("SELECT p.codigo, p.nombre, p.precio_usd, p.stock, COALESCE(p.stock_minimo,0), COALESCE(p.created_at,''), p.categoria_id, c.nombre, c.color FROM productos p LEFT JOIN categorias c ON c.id = p.categoria_id ORDER BY p.nombre ASC")
        .map_err(|e| e.to_string())?;
