...
function renderProductSearch() {
  ...
  tbody.innerHTML = '';
  if (!query) return;
  const filtered = productCache.filter(p => p.nombre.toLowerCase().includes(query) || p.codigo.toLowerCase().includes(query));
...
function showProductDetail(codigo) {
  ...
  document.getElementById('detail-codigo').textContent = p.codigo;
...
function openNewProductModal() {
  ...
  ['product-codigo','product-nombre','product-precio','product-stock'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('product-categoria').value = '';
  document.getElementById('product-codigo').disabled = false;
  document.getElementById('product-codigo-group').style.display = 'block';
...
function editProduct(codigo) {
  ...
  document.getElementById('product-codigo-group').style.display = 'none';
...
async function saveProduct() {
  const codigo = editingProduct || document.getElementById('product-codigo').value.trim();
  ...
  if (!codigo) { showToast('Complete todos los campos', 'error'); return; }
  try {
    if (editingProduct) {
    ...
    if (!editingProduct && e.includes('UNIQUE')) {
      showToast('Este código de producto ya está registrado', 'error');
    } else {
...
async function loadDailySummary() {
  try {
    ...
    summary.ventas.forEach(v => {
      const tr = document.createElement('tr');
      tr.innerHTML = '<td>' + v.id + '</td><td>' + v.fecha_hora.split(' ')[1] + '</td><td>' + v.username + '</td><td>' + formatMetodoLabel(v.metodo_pago) + '</td><td>' + formatUSD(v.total_usd) + '</td><td>' + formatBS(v.total_bs) + '</td>';
      frag.appendChild(tr);
    });
    tbody.appendChild(frag);
  ...
  try {
    ...
    if (reportData.por_metodo && reportData.por_metodo.length) {
      ...
      reportData.por_metodo.forEach(m => {
        ...
        html += '<p>' + label + ': ' + formatUSD(m.total_usd) + ' / ' + formatBS(m.total_usd * tasaActual) + '</p>';
      });
    }
...
function printCloseReport() {
  ...
  d.por_metodo.forEach(m => {
    doc.write('<p>' + formatMetodoLabel(m.metodo) + ': ' + formatUSD(m.total_usd) + '</p>');
  });
  doc.write('<hr>');
...
async function showCierreDetalle(cierreId) {
  try {
    ...
    if (d.por_metodo && d.por_metodo.length) {
      ...
      d.por_metodo.forEach(m => {
        html += '<p>' + formatMetodoLabel(m.metodo) + ': ' + formatUSD(m.total_usd) + '</p>';
      });
    }
